...
MIN_CLIP_SECONDS = float(os.getenv("HAPPYCLIPS_MIN_CLIP_SECONDS", "8"))
...
@app.post("/process")
def process_video(request: ProcessRequest, background_tasks: BackgroundTasks) -> dict[str, Any]:
...
def select_moments(transcript: list[dict[str, Any]]) -> list[dict[str, float]]:
    ...
    prompt = (
        ...
        "with start and end timestamps. Clips must be 8-25 seconds, contain complete "
        "ideas, and be ordered from strongest to weakest. Transcript:\n"
