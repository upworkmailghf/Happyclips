const steps = [
  ['transcription', 'Captions or audio transcript'],
  ['ai_analysis', 'AI analyzing viral moments'],
  ['downloading_clips', 'Downloading selected sections'],
...
function renderSteps(activeStep, status = 'processing') {
  const activeIndex = steps.findIndex(([key]) => key === activeStep);
  stepList.innerHTML = steps.map(([key, label], index) => {
    const isDone = activeStep === 'completed' || (activeIndex >= 0 && index < activeIndex);
    const isActive = activeIndex === index && status !== 'failed';
    const isFailed = activeIndex === index && status === 'failed';
    const icon = isDone ? '✔' : isFailed ? '!' : isActive ? '→' : '•';
    const className = isDone ? 'done' : isFailed ? 'failed' : isActive ? 'active' : '';
...
function startPolling() {
  stopPolling();
  state.pollTimer = window.setInterval(() => pollStatus().catch((error) => showError(error.message)), 2500);
}
